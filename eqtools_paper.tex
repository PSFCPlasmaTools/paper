...
The new \emph{a priori} inclusion of $\boldsymbol{B}$ in the spline calculation matrix $\boldsymbol{A_{inv}}$ removes the necessary derivative generation and reduces total computation by half \textcolor{red}{(denoted $\boldsymbol{A_{inv}^*}$)}\gnote{I don't know where this parenthetical comment belongs, but probably not here.}. 
