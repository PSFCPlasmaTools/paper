...
This package, developed in the open-source Python programming language and freely distributed via Github \note{need a way to link!}, presents a consistent, user-friendly interface independent of data source or storage method, replacing machine- and storage-specific methods which are often nonintuitive to use (requiring, for example, manual hooks into data storage for derived quantities) and which often use a static code structure that can be difficult to extend for experimental data from multiple machines, storage methods, or reconstruction codes.\gnote{too harsh?}
